...
public class MainForm : Form
{
    ...
    private static GuiParser m_GuiContainer = new GuiParser();

    private IQComm m_BoardComm = new IQComm();

    private IQUart m_BoardUart = new IQUart();

    private int m_GuiBorderWidth = Settings.Default.GuiBorderWidth;

    private int m_GuiRwButtonSpace = Settings.Default.GuiRWButtonSpace;
    ...
    private ushort m_BinChecksumVer = 1;

    private uint videomagicKey = 2036624993u;

    private uint SaveBinFile_Interval = 5u;

    private uint SaveBinFile_Interval_sec;

    private List<string> m_PluginList = new List<string>();

    private FormLiveView m_fmLiveView;

    private ConnectSetting connSetting = new ConnectSetting();
    ...
    private IQConfig m_json_config = new IQConfig();

    private Dictionary<ulong, string> m_chipdef = new Dictionary<ulong, string>();
    ...
    private unsafe void burnFWToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        _ = new byte[260];
        _ = new byte[100];
        ShowVideoDevDlg(msg, 260);
        if (AITAPI_OpenDeviceByPath(msg, &in_ait_handel) != 0)
        {
            MessageBox.Show("Oepn device failed!");
        }
        if (ToBurnFW(in_ait_handel) != 0)
        {
            MessageBox.Show("FW update error!");
        }
        else
        {
            MessageBox.Show("FW Update OK!");
        }
    }
    ...
    private unsafe void sendIQDataToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        num = array.Length;
        if (Send_IQData(in_ait_handel, array, num) != 0)
        {
            MessageBox.Show("Send IQ Data error!");
        }
        else
        {
            MessageBox.Show("Send IQ Data OK!");
        }
    }
    ...
    private unsafe void getIQDataToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        MessageBox.Show("Get IQ Data OK!");
    }
    ...
    public unsafe int Get_IQData(IntPtr in_ait_handel, ushort CmdData_Size, byte[] CmdData_Data, ushort Out_IQ_Size, byte[] Out_IQData)
    {
        ...
        return AITAPI_ReadPartialFlashData(in_ait_handel, 0u, Out_IQ_Size, Out_IQData, byteRet, null, null);
    }
    ...
    private string btOpenFwFile_Click()
    {
        ...
        openFileDialog.Title = "Select FW bin file";
    ...
    private int LoadConfig()
    {
        ...
        if (!File.Exists(text))
        {
            MessageBox.Show("config not exist " + text);
            return -1;
        }
    ...
    {
        string text = "";
        if (string.IsNullOrEmpty(m_chipdef[(ulong)chipId]))
        {
            return text + "isp_api.xml";
        }
        return text + m_chipdef[(ulong)chipId] + "_isp_api.xml";
    }
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        ...
        m_GuiContainer.chooseProduct(out product_style);
    ...
    {
        string text = "";
        text = m_BoardComm.Connect(clientHostName, Port, Settings.Default.ConnectionTimeout);
        if (!text.Equals(""))
        {
    ...
    private void ClientDisconnect()
    {
        ...
        toolBtnConnect.Image = GuiParser.GetImageByName("connect.png");
    ...
    public void CheckConnection()
    {
        ...
        {
            Invoke(new UpdateDisconnectHandler(ClientDisconnect));
        }
    }
    ...
    private void SetCommonConfig()
    {
        ...
        SetSensorPadIDByAPI(sensorPadID);
    ...
    private void ConnectToBoardFinish(string strMsg)
    {
        ...
        {
            Invoke(new ShowMessageHandler(GUIShowMessage), "Not find chip id by API. Load default isp_api.xml file\n");
        ...
        {
            btnUartConn.Enabled = true;
            Function chipFunction = new Function();
            GetChipFunction(m_chipID, m_BoardComm.ConnectMode, ref chipFunction);
            ...
            if (m_chipID != ChipID.M5 && m_chipID != ChipID.M5U && m_chipID != ChipID.I2 && m_chipID != ChipID.I6)
            {
        ...
        {
            btnUartConn.Enabled = true;
            toolBtnConnect.Image = GuiParser.GetImageByName("disconnect.png");
            ...
            Update();
            Function chipFunction2 = new Function();
            GetChipFunction(m_chipID, m_BoardComm.ConnectMode, ref chipFunction2);
    ...
    public string ReadXMLGroup(IQComm comm, FILE_TRANSFER_ID type, string srcpath)
    {
        ...
        if (comm.ReceiveXMLApiPacket(this, apiId, out pbyRcvData) > 0)
        {
        ...
        {
            result = "Warning!! " + chipname + " ISP API XML on board not found. " + Name + " receive packet fail !! ";
        }
        return result;
    }
    ...
    public string ReadXMLGroup(IQComm comm, FILE_TRANSFER_ID type, string srcpath, ushort APIVerMajor, ushort APIVerMinor)
    {
        string result = "";
        short apiId = (short)type;
        if (comm.ReceiveXMLApiPacket(apiId, out var pbyRcvData, APIVerMajor, APIVerMinor) > 0)
        {
            byte[] array = new byte[pbyRcvData.Length - 8];
            for (int i = 8; i < pbyRcvData.Length; i++)
            {
    ...
    private void ConnectToBoardTask(object objList)
    {
        ...
        Invoke(new UpdateConnectFinishHandler(ConnectToBoardFinish), text);
    }
    ...
    private void GetBoardXMLTask(object objList)
    {
        ...
        if (!text.Equals(""))
        {
            MessageBox.Show(text, "Connection", MessageBoxButtons.OK, MessageBoxIcon.Hand);
        }
        else
        {
            GetXML(Application.StartupPath + "\\I3_isp_api.xml");
        }
        if (m_BoardComm.IsConnected())
        {
            m_BoardComm.Disconnect();
        ...
        if (!m_BoardComm.IsConnected())
        {
            if (Regex.IsMatch(text, pattern) || Regex.IsMatch(text, pattern2))
            {
                ...
                if (!text2.Equals(""))
                {
                    ...
                    if (0 <= num && num <= 65535)
                    {
                        ConnectSetting connectSetting = new ConnectSetting(text, num);
                        Thread thread = new Thread(ConnectToBoardTask);
                        ...
                        {
                            btnUartConn.Enabled = true;
                            MessageBox.Show("ID should be 1000 ~ 9999", "Status", MessageBoxButtons.OK);
                        }
                    }
                    else
                    {
                        btnUartConn.Enabled = true;
                        MessageBox.Show("Port Number should be 0 ~ 65535", "Status", MessageBoxButtons.OK);
                    }
                }
                else
                {
                    btnUartConn.Enabled = true;
                    MessageBox.Show("Port Number should not be empty", "Status", MessageBoxButtons.OK);
                }
            }
            else
            {
                btnUartConn.Enabled = true;
                MessageBox.Show("Host Name should be legal IP address or Domain name", "Status", MessageBoxButtons.OK);
            }
        }
        else
        {
    ...
    private void toolTxbClientHostName_KeyPress(object sender, KeyPressEventArgs e)
    {
        ...
        if (keyChar >= '0' && keyChar <= '9' || 'a' <= keyChar && keyChar <= 'z' || keyChar == '.' || keyChar == '-' || keyChar == '\b')
        {
            e.Handled = false;
        }
        else
    ...
    private void GetSensorInfoByChipID()
    {
        ...
        num2 = m_BoardComm.GetApiIDValue(m_chipID, "QuerySensorInfo");
    ...
    private void GetChipIDByXml(string xmlPath)
    {
        ...
        else
        {
            ...
            Invoke(new ShowMessageHandler(GUIShowMessage), text);
        }
    }
    ...
    private void GetApiVersion()
    {
        ...
        if (m_BoardComm.ReceiveApiPacket(apiIDValue, out var pbyRcvData) > 0)
        {
    ...
    {
        byte[] pbyRcvData = null;
        if (m_BoardComm.ReceiveApiPacket(APIVersion, out pbyRcvData) > 0)
        {
            short apiIDValue = m_BoardComm.GetApiIDValue(ChipID.I1, "ApiVersion");
    ...
    private void SetApiVersion(ApiVersion version)
    {
        ...
        {
            Invoke(new ChangeApiVersionHandler(ChangeXmlType), m_chipID, version);
        }
    }
    ...
    private void ChangeLoadBinXmlType(ChipID chipId)
    {
        try
        {
            if (Path.GetExtension(loadbinxml).Equals(".bin"))
            {
    ...
    private void SuccessGetXml()
    {
        ...
        text = Application.StartupPath + "\\CvtXml";
        ...
        {
            string msgText = "Warning!! " + chipname + " ISP API XML on board not found. " + Name + " receive packet fail !! ";
            m_GuiContainer.ShowMessage("", msgText);
            return;
        }
        if (m_GuiContainer.checkChipID(m_chipID, desxml) != 1)
        {
            m_GuiContainer.checkAPIVersion(desxml, m_chipID, connectMode);
            m_GuiContainer.InitGUI(desxml);
        }
    }
    ...
    private void FailGetXml(string strmsg)
    {
        ...
        else if (File.Exists(srcxml))
        {
            ...
            {
                m_GuiContainer.checkAPIVersion(srcxml, m_chipID, connectMode);
                m_GuiContainer.InitGUI(srcxml);
            }
        }
        else
        {
            MessageBox.Show("Xml cannot be found.", Text, MessageBoxButtons.OK);
        }
    }
    ...
    private void GetXMLTask(object obj)
    {
        ...
        if (!text.Equals(""))
        {
            Invoke(new FailGetXmlHander(FailGetXml), text);
        }
        else
        {
            Invoke(new SuccessGetXmlHander(SuccessGetXml));
        }
    }
    ...
    private void ChangeUartXmlType(ChipID chipId, ApiVersion version)
    {
        ...
        switch (chipId)
        {
            ...
            default:
                ...
                {
                    MessageBox.Show("[Error] Not support Chip Type!");
                    break;
                }
                getChipXmlName(ref srcxml, ref desxml);
    ...
    private void ChangeXmlType(ChipID chipId, ApiVersion version)
    {
        ...
        text = Application.StartupPath + "\\" + m_chipdef[(ulong)chipId] + "_isp_api.xml";
        text2 = Application.StartupPath + "\\CvtXml\\" + m_chipdef[(ulong)chipId] + "_isp_api_" + EvbAPIVerMajor + "_" + EvbAPIVerMinor + ".xml";
        text4 = Application.StartupPath + "\\CvtXml";
        ...
        if (GetXML(text))
        {
            ...
            {
                m_GuiContainer.checkAPIVersion(text2, m_chipID, connectMode);
                m_GuiContainer.InitGUI(text2);
            }
        }
        ...
        else if (File.Exists(text))
        {
            ...
            {
                m_GuiContainer.checkAPIVersion(text, m_chipID, connectMode);
                m_GuiContainer.InitGUI(text);
            }
        }
        else
    ...
    public void GetImageTask(object obj)
    {
        ...
        if (text.Equals(""))
        {
            Invoke(new ShowMessageHandler(GUIShowMessage), "Get image success" + Environment.NewLine + AppDomain.CurrentDomain.SetupInformation.ApplicationBase + "image");
        }
        else
        {
            Invoke(new ShowMessageHandler(ShowMessageBox), "Get image failed");
            Invoke(new ShowMessageHandler(GUIShowMessage), text);
        }
        CheckConnection();
    }
    ...
    private void SaveImageFromCamera(CAMERA_MODE_TYPE outputType)
    {
        if (m_BoardComm.IsConnected())
        {
            ...
            if (m_BoardComm.ConnectMode == CONNECT_MODE.MODE_SOCKET || m_BoardComm.ConnectMode == CONNECT_MODE.MODE_USB || m_BoardComm.ConnectMode == CONNECT_MODE.MODE_UART)
            {
        ...
        {
            MessageBox.Show(Settings.Default.MsgNotConnected, "Connection", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }
    }
    ...
    private bool getCameraHdrMode()
    {
        bool result = false;
        byte[] pbyRcvData = null;
        if (m_BoardComm.IsConnected() && m_BoardComm.ReceivePacket(CAMERA_CMD_TYPE.CAMERA_CMD_GET_MODE, out pbyRcvData) > 0 && pbyRcvData[0] == 1)
        {
            result = true;
        }
        return result;
    ...
    private void SetRawCount(uint frame_count, uint loop_count)
    {
        ...
        {
            MessageBox.Show("HDR Mode Raw Data Numbers Must Be Even", Text, MessageBoxButtons.YesNo);
            return;
        }
    ...
    {
        FormSetRawFrameCount formSetRawFrameCount = new FormSetRawFrameCount(m_RawCount, m_RawLoopCount, SetRawCount);
        formSetRawFrameCount.StartPosition = FormStartPosition.CenterScreen;
    ...
    private void toolGetScalarOutImage_Click(object sender, EventArgs e)
    {
        ...
        {
            FormSclSetting formSclSetting = new FormSclSetting(m_SclDevice, m_SclChannel, m_SclPort, SetSclSetting);
            formSclSetting.StartPosition = FormStartPosition.CenterScreen;
            formSclSetting.Show();
        }
        else
        {
    ...
    private void SaveTunningParamters()
    {
        ...
        saveFileDialog.Filter = "Parameters file (*.xml) | *.xml";
        ...
        if (!saveFileDialog.FileName.Equals(""))
        {
            if (text.Equals(""))
            {
                ...
                MessageBox.Show(text, "Save Parameters File", MessageBoxButtons.OK);
                m_GuiContainer.isFWAPIParmChange = false;
            }
            else
            {
    ...
    private void SaveTunningParmToBin(bool isSaveBinXml)
    {
        ...
        if (saveFileDialog.ShowDialog() == DialogResult.OK)
        {
            try
            {
                ...
                BinaryWriter binaryWriter = new BinaryWriter(File.Open(saveFileDialog.FileName, FileMode.Create));
                byte[] array3 = null;
                if (isSaveBinXml)
                {
        ...
        if (!saveFileDialog.FileName.Equals(""))
        {
            MessageBox.Show("Save File " + saveFileDialog.FileName + " Success.", "Save Parameters File", MessageBoxButtons.OK);
            m_GuiContainer.isFWAPIParmChange = false;
        }
    }
    ...
    {
        FormSetMagicKey formSetMagicKey = new FormSetMagicKey(magicKey, SetMagicKey);
        formSetMagicKey.StartPosition = FormStartPosition.CenterScreen;
    ...
    public void ListPluginFile()
    {
        ...
        for (int i = 0; i < m_PluginList.Count; i++)
        {
            ...
            {
                toolCbxPlugIn.Items.Add(versionInfo.ProductName);
            }
            else
            {
        ...
        try
        {
            if (text == "AwbAnalyzerCombo.FormAwbAnalyzerCombo")
            {
                ...
                if (@event != null)
                {
                    ...
                    addMethod.Invoke(form, parameters);
    ...
    public void GetIQIndexTask(object obj)
    {
        ...
        {
            case CONNECT_MODE.MODE_USB:
                m_BoardComm.ReceiveUSBInitialApiPacket(apiId, bufferInitial, out pbyRcvData);
                break;
            case CONNECT_MODE.MODE_UART:
                m_BoardComm.ReceiveUartApiPacket(apiId, out pbyRcvData);
                break;
            case CONNECT_MODE.MODE_SOCKET:
                m_BoardComm.ReceiveApiPacket(apiId, out pbyRcvData);
                break;
        }
        PacketParser.ConvertBufferToIQIndex(ref IQIndex, pbyRcvData);
        Invoke(new ShowMessageHandler(ShowIQIndex), IQIndex.ToString());
    }

    private void panelMainContent_MouseClick(object sender, MouseEventArgs e)
    {
        panelMainContent.Focus();
    }
    ...
    {
        btnWritePage.Enabled = action == PAGE_ACTION.W || action == PAGE_ACTION.RW ? true : false;
        btnReadPage.Enabled = action == PAGE_ACTION.R || action == PAGE_ACTION.RW ? true : false;
    }
    ...
    {
        FormConvert formConvert = new FormConvert();
    ...
    private void btnUartConn_Click(object sender, EventArgs e)
    {
        if (m_BoardComm.ConnectMode == CONNECT_MODE.MODE_UART)
        {
            if (!m_BoardUart.IsConnected())
            {
                ...
                toolBtnConnect.Enabled = false;
                btnUartConn.Image = GuiParser.GetImageByName("transfer.png");
                Invoke(new UpdateConnectFinishHandler(ConnectToBoardFinish), "Connect UART Comm.!");
            }
            else
            {
                toolBtnConnect.Enabled = true;
                m_BoardComm.disconnectUartPage();
                m_BoardUart.DisConnected();
                btnUartConn.Image = GuiParser.GetImageByName("refresh.png");
                MessageBox.Show("DisConnect Uart!");
            }
            return;
        }
        ...
        if (text2 == "")
        {
            if (m_BoardComm.IsConnected())
            {
                toolBtnConnect.Enabled = true;
                btnUartConn.Image = GuiParser.GetImageByName("refresh.png");
                Invoke(new UpdateConnectFinishHandler(ConnectToBoardFinish), "Connect USB Comm.!");
            }
            else
            {
                toolBtnConnect.Enabled = false;
                btnUartConn.Image = GuiParser.GetImageByName("transfer.png");
                MessageBox.Show("DisConnect USB Comm.!");
            }
        }
        else
        {
            MessageBox.Show("[Error] USB Connect message : %s", text2);
        }
    }

    private void uARTLogToolStripMenuItem_Click(object sender, EventArgs e)
    {
        FormUART formUART = new FormUART();
        formUART.ShowDialog();
        formUART.Focus();
    }
    ...
    {
        FormAutoSaveBinFile formAutoSaveBinFile = new FormAutoSaveBinFile(SaveBinFile_Interval, autoSaveBin);
        formAutoSaveBinFile.StartPosition = FormStartPosition.CenterScreen;
    ...
    {
        if (m_chipdef.ContainsKey((ulong)chipId))
        {
            chipName = m_chipdef[(ulong)chipId];
        }
        else
        {
            chipName = "UNKNOW_" + chipId;
        }
    }
    ...
    public int GetChipFunction(ChipID chipId, CONNECT_MODE connect_mode, ref Function chipFunction)
    {
        int result = -1;
        string text = "";
        text = GetConnectModeString(connect_mode);
        Chiplist[] chipList = m_json_config.ChipList;
        foreach (Chiplist chiplist in chipList)
        {
    ...
    private void InitializeComponent()
    {
        ...
        btnPageItemCollapse.Image = GuiParser.GetImageByName("arrowLeft.png");
        ...
        treeNode.Name = "Node0";
        treeNode.Text = "Node0";
        treePageItem.Nodes.AddRange(new TreeNode[1] { treeNode });
        ...
        btnPageItemExpand.Image = GuiParser.GetImageByName("arrowRight.png");
        ...
        toolTxbClientHostName.Text = "172.19.10.187";
        ...
        toolBtnStepBack.Enabled = false;
        toolBtnStepBack.Image = GuiParser.GetImageByName("back.png");
        ...
        toolBtnStepNext.Enabled = false;
        toolBtnStepNext.Image = GuiParser.GetImageByName("next.png");
        ...
        toolBtnReadAll.Image = GuiParser.GetImageByName("toolBtnReadAll.png");
        ...
        toolBtnWriteAll.Image = GuiParser.GetImageByName("toolBtnWriteAll.png");
        ...
        toolBtnGetRawImage.Image = GuiParser.GetImageByName("rawimage.png");
        ...
        toolBtnGetRawStream.Enabled = false;
        toolBtnGetRawStream.Image = GuiParser.GetImageByName("rawstream.png");
        ...
        toolBtnGetIspYuv.Image = GuiParser.GetImageByName("yuvimage.png");
        ...
        toolBtnGetJpeg.Image = GuiParser.GetImageByName("jpgimage.png");
        ...
        toolGetScalarOutImage.Image = GuiParser.GetImageByName("scaleimage.png");
