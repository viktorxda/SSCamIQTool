...
public class FormLineChart : Form
{
    ...
    private IQComm m_Comm;

    private GuiItem m_Item;

    private GuiGroup m_Group;

    private bool m_bAutoWrite = true;

    private Chart GridChart = new Chart();
    ...
    private void InitChart()
    {
        ...
        new Series("SeriesLine");
        GridChart.Series.Add("SeriesLine");
    ...
    private void UpdateChart()
    {
        ...
        {
            GridChart.Series["SeriesLine"].Points.AddXY(i, m_Item.DataValue[selectedIndex * m_Item.XSize + i]);
            ...
            Point item = new Point(i, (int)m_Item.DataValue[selectedIndex * m_Item.XSize + i]);
    ...
    private void LineChart_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        {
            FeaturePoint[m_curIdx] = new Point(FeaturePoint[m_curIdx].X, YMin);
        }
        else if (axisValuesFromMouse.Item2 > YMax)
        {
            FeaturePoint[m_curIdx] = new Point(FeaturePoint[m_curIdx].X, YMax);
        }
        else
        {
            FeaturePoint[m_curIdx] = new Point(FeaturePoint[m_curIdx].X, (int)Math.Round(axisValuesFromMouse.Item2));
        }
        ...
        {
            GridChart.Series["SeriesLine"].Points.AddXY(item.X, item.Y);
