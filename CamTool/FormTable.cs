...
public class FormTable : Form
{
    ...
    private static readonly MainForm undoredoParam = new();
    ...
    public FormTable(GuiGroup group, IQComm comm, string itemTag, bool bAW)
    {
        ...
        if (guiItem.Text is "Manual.Luma.Lut" or "Manual.Hue.Lut" or "Manual.Sat.Lut" or "Auto.Luma.Lut" or "Auto.Hue.Lut" or "Auto.Sat.Lut")
        {
        ...
        Width = num6 > m_nDefaultFormWdith
            ? m_nDefaultFormWdith
            : num6 < labelShowTableTitle.Width + (labelShowTableTitle.Location.X * 2)
        ...
        Height = num7 > m_nDefaultFormHeight ? m_nDefaultFormHeight : num7;
    ...
    private void btnImportTable_Click(object sender, EventArgs e)
    {
        ...
        {
            Filter = "data file (*.txt)|*.txt|data file (*.lut)|*.lut",
    ...
    {
        StreamWriter streamWriter = new(strFileName);
        for (int i = 0; i < dataGridShowTable.Rows.Count; i++)
        {
            for (int j = 0; j < dataGridShowTable.Rows[i].Cells.Count; j++)
            {
    ...
    {
        return a.Key < b.Key ? -1 : a.Key == b.Key && a.Value < b.Value ? -1 : 1;
    }
    ...
    private void FormTable_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < array.Count(); i++)
            {
                ...
                {
                    if (int.TryParse(array2[j], out int result))
                    {
                        list2.Add(result);
                    }
                }
            ...
            List<KeyValuePair<int, int>> list3 = new();
            ...
            int num = 0;
            List<KeyValuePair<int, int>> list4 = new();
            List<List<KeyValuePair<int, int>>> list5 = new();
