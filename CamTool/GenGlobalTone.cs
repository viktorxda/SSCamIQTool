...
internal class GenGlobalTone
{
    ...
    private readonly List<double> m_genFcureLeft = new();

    private readonly List<double> m_genFcureRight = new();

    private readonly List<double> m_genBlending = new();

    private readonly List<double> m_genFcureFinal = new();
    ...
    public void CalcFcureLeft()
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            double num3 = m_genX[i];
            double num4 = num != 0.0 ? !(num > 0.0) ? num2 * Math.Log10(1.0 - ((Math.Log10(1.0 - (num3 / 65536.0) - (num / 65536.0)) - Math.Log10((0.0 - num) / 65536.0)) / (Math.Log10(1.0 - (num / 65536.0)) - Math.Log10((0.0 - num) / 65536.0)))) : num2 * (Math.Log10((num3 / 65536.0) + (num / 65536.0)) - Math.Log10(num / 65536.0)) / (Math.Log10(1.0 + (num / 65536.0)) - Math.Log10(num / 65536.0)) : num2 * num3 / 65536.0;
