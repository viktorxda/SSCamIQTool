...
public class FormConvert : Form
{
    ...
    private readonly GuiParser m_GuiContainer = new();

    private readonly FormConvertList formcvtlist = new();
    ...
    private void SaveCvtDesTunningParamters(string Src)
    {
        if (Src == "")
        {
            _ = MessageBox.Show("Please Select Conver Source File (bin or xml).", "Conver Source File", MessageBoxButtons.OK);
            return;
        }
        string text2 = "";
        if (Src != "")
        ...
        string text = m_GuiContainer.SaveCvtDesXml(text2);
    ...
    private void SaveCvtDesToBin()
    {
        ...
        BinaryWriter binaryWriter = new(File.Open(text, FileMode.Create));
        binaryWriter.Write(cvtDesBinBytes);
        binaryWriter.Close();
        if (!text.Equals(""))
        {
            _ = MessageBox.Show("Save File " + text + " Success.", "Save Parameters File", MessageBoxButtons.OK);
        }
    }
    ...
    private void SaveCvtDesToBinXml(string Src)
    {
        if (Src == "")
        {
            _ = MessageBox.Show("Please Select Conver Source Bin File.", "Conver Source File", MessageBoxButtons.OK);
            return;
        }
        string text = Path.GetDirectoryName(Src) + "/" + Path.GetFileNameWithoutExtension(Src) + "_ConverTo_" + formcvtlist.comboBoxCvtXml.SelectedItem.ToString() + ".bin";
        try
        {
            uint magicKey = 1234u;
            ...
            _ = m_GuiContainer.SaveCvtDesXml(text2);
        ...
        if (text.Equals(""))
        {
            _ = MessageBox.Show("Save File " + text + " Fail.", "Save Parameters File", MessageBoxButtons.OK);
        }
    }
    ...
    private void GetXML()
    {
        ...
        if (!text2.Equals(""))
        {
            _ = MessageBox.Show(text2, "Get Xml On Board", MessageBoxButtons.OK);
        }
        else
        {
            _ = m_GuiContainer.InitCvtDesGUI(text);
        }
    }
    ...
    private void btnConvert_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (formcvtlist.radioBtnSelCvtXml.Checked)
            {
                ...
                {
                    _ = MessageBox.Show("Xml in CvtXml Folder Cannot Be Found.", "Conver Source File", MessageBoxButtons.OK);
                    return;
                }
                ...
                if (!File.Exists(text2))
                {
                    _ = MessageBox.Show(text2 + " Cannot Be Found.", "Conver Source File", MessageBoxButtons.OK);
                    return;
                }
                m_CvtDesFile = text2;
                _ = m_GuiContainer.InitCvtDesGUI(text2);
            }
            ...
            for (int i = 0; i < MultiSrcBin.Length; i++)
            {
                ...
                {
                    _ = m_GuiContainer.InitFileGUI(i, MultiSrcBin[i]);
                    _ = m_GuiContainer.InitCvtDesGUI(m_CvtDesFile);
            ...
            for (int j = 0; j < MultiSrcXml.Length; j++)
            {
                ...
                {
                    _ = m_GuiContainer.InitFileGUI(j + MultiSrcBin.Length, MultiSrcXml[j]);
                    _ = m_GuiContainer.InitCvtDesGUI(m_CvtDesFile);
                    m_GuiContainer.copyCvtSrcToCvtDes();
                    if (formcvtlist.radioBtnCvtToBin.Checked)
                    {
    ...
    private void SelSrcXml()
    {
        ...
        if (openFileDialog.ShowDialog() == DialogResult.OK)
        {
            ...
            _ = m_GuiContainer.InitCvtSrcGUI(openFileDialog.FileName);
        }
    }
    ...
    private void SelSrc()
    {
        OpenFileDialog openFileDialog = new()
        {
            Filter = "Parameters file  Bin file  (*.bin)| *.bin| All files (*.*)|*.*",
            InitialDirectory = Application.StartupPath
        };
        ...
        if (m_GuiContainer.LoadCvtSrcBinXmlFile(openFileDialog.FileName) != 0)
        {
            ...
            if (File.Exists(text))
            {
                _ = m_GuiContainer.InitCvtSrcGUI(text);
                m_GuiContainer.LoadCvtSrcBinFile(openFileDialog.FileName);
            }
            else
            {
    ...
    private void SelMultiSrc()
    {
        OpenFileDialog openFileDialog = new()
        {
            Filter = "Bin file  (*.bin)| *.bin| Parameters file (*.xml)| *.xml| All files (*.*)|*.*",
            InitialDirectory = Application.StartupPath,
            Multiselect = true
        };
        if (openFileDialog.ShowDialog() != DialogResult.OK)
        ...
        for (int j = 0; j < openFileDialog.FileNames.Length; j++)
        {
            if (Path.GetExtension(openFileDialog.FileNames[j]).Equals(".bin"))
            {
                ...
                {
                    _ = m_GuiContainer.InitFileGUI(j, openFileDialog.FileNames[j]);
                }
            }
    ...
    private void SelDesXml()
    {
        ...
        if (openFileDialog.ShowDialog() == DialogResult.OK)
        {
            _ = m_GuiContainer.InitCvtDesGUI(openFileDialog.FileName);
        }
    }
    ...
    private void FormConvert_Load(object sender, EventArgs e)
    {
        ...
        formcvtlist.radioBtnSelXmlOnBoard.Enabled = m_DbgParser.IsConnected();
    }
    ...
    {
        TreeNode treeNode = new("Node0");
        TreeNode treeNode2 = new("Node0");
        TreeNode treeNode3 = new("Node0");
