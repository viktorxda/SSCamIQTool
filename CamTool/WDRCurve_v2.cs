...
{
    private readonly IQComm comm;

    private static GuiGroup gammaGroup;

    private static GuiGroup gammaGroup_temp;

    private static FormGlobalToneVer1 saveGlobalToneVer1 = new();

    private static FormGlobalToneGammaVer1 saveGlobalToneGammaVer1 = new();

    private static FormGlobalToneDeGammaVer1 saveGlobalToneDeGammaVer1 = new();

    private static FormGlobalTone saveGlobalTone = new();

    private static FormGlobalToneGamma saveGlobalToneGamma = new();

    private static FormGlobalToneDeGamma saveGlobalToneDeGamma = new();
    ...
    public WDRCurve_v2(GuiGroup group, IQComm comm)
    {
        ...
        List<string> nameList = new() { "API_WDRCurveFull_OP_TYPE_GlobalTone", "API_WDRCurveFull_AUTO_GlobalToneLut", "API_WDRCurveFull_MANUAL_GlobalToneLut" };
        List<string> nameList2 = new() { "API_WDRCurveFull_OP_TYPE_Gamma", "API_WDRCurveFull_AUTO_GammaLut", "API_WDRCurveFull_MANUAL_GammaLut" };
        List<string> nameList3 = new() { "API_WDRCurveFull_OP_TYPE_DeGamma", "API_WDRCurveFull_AUTO_DeGammaLut", "API_WDRCurveFull_MANUAL_DeGammaLut" };
        if (enableGenerateCurve == 1)
        {
            if (CheckItemTag(group, nameList))
            {
                FormGlobalToneVer1 value = new(gammaGroup_temp)
                {
            ...
            {
                FormGlobalToneGammaVer1 value2 = new(gammaGroup_temp)
                {
                    FormBorderStyle = FormBorderStyle.None,
                    TopLevel = false,
                    Visible = true,
            ...
            {
                FormGlobalToneDeGammaVer1 value3 = new(gammaGroup_temp)
                {
                    FormBorderStyle = FormBorderStyle.None,
                    TopLevel = false,
                    Visible = true,
        ...
        else
        {
            if (CheckItemTag(group, nameList))
            {
                FormGlobalTone value4 = new(gammaGroup_temp)
                {
                    FormBorderStyle = FormBorderStyle.None,
                    TopLevel = false,
                    Visible = true,
            ...
            if (CheckItemTag(group, nameList2))
            {
                FormGlobalToneGamma value5 = new(gammaGroup_temp)
                {
                    FormBorderStyle = FormBorderStyle.None,
                    TopLevel = false,
                    Visible = true,
            ...
            if (CheckItemTag(group, nameList3))
            {
                FormGlobalToneDeGamma value6 = new(gammaGroup_temp)
                {
                    FormBorderStyle = FormBorderStyle.None,
                    TopLevel = false,
                    Visible = true,
