...
public class FormGlobalToneDeGamma : Form
{
    ...
    private readonly int m_nDeGammaLength = 33;

    private readonly int m_nDeGammaInterval = 128;

    private readonly short m_nDeGammaValueMax;
    ...
    private readonly int nYMax = 4096;

    private readonly int DeGammaNumber = 33;

    private readonly int NumberOfGroup = 16;

    private static readonly int[] xnode_Auto = new int[528];

    private static readonly int[] ynode_Auto = new int[528];

    private static readonly int[] xnode_Manual = new int[33];

    private static readonly int[] ynode_Manual = new int[33];
    ...
    private static readonly int[] RecordDeGlobalToneIndex_Auto = new int[528];

    public static long[] DeGammainitialvalueR_Manual;

    private static readonly int[] RecordDeGlobalToneIndex_Manual = new int[33];
    ...
    private void DeGammacheckBoxEnable_CheckedChanged(object sender, EventArgs e)
    {
        ...
        chartDeGamma.Enabled = DeGammaEnable != 0;
    }
    ...
    {
        getRedDeGammaArray(out long[] pDeGammaR);
    ...
    public FormGlobalToneDeGamma(GuiGroup group)
    {
    ...
    {
        int num = 33;
        GetDeGammaWithOB(m_nDeGammaValueMax, m_nDeGammaValueMax, 0m, out Point[] pDeGammaArray);
        ...
        if (pDeGammaArray != null)
        {
            ...
            {
                _ = chartDeGamma.Series["SeriesDeGamma"].Points.AddXY(i, pDeGammaArray[i].Y);
            }
        ...
        for (int j = 0; j < 512; j++)
        {
            _ = chartDeGamma.Series.Add("SeriesBezierControl" + j);
    ...
    public void InitBezierControl(Series seiresRLine, Series seiresRPoints, List<Point> m_ptFeatureR, List<Point> m_ptFeatureRControl)
    {
        ...
        for (int j = 0; j < m_ptFeatureRControl.Count; j++)
        {
            ...
            if (j % 2 == 0)
            {
                ...
                {
                    num27 = (int)((a * k) + b);
                    _ = chartDeGamma.Series["SeriesBezierControl" + j].Points.AddXY(k, num27);
                }
            }
            else
            {
                ...
                {
                    num27 = (int)((a * l) + b);
                    _ = chartDeGamma.Series["SeriesBezierControl" + j].Points.AddXY(l, num27);
                }
            }
        }
    }
    ...
    public void drawControlLine(List<Point> m_ptFeature, List<Point> m_ptFeatureControl)
    {
        for (int i = 0; i < m_ptFeatureControl.Count; i++)
        {
            ...
            if (i % 2 == 0)
            {
                ...
                {
                    num5 = (int)((a * j) + b);
                    _ = chartDeGamma.Series["SeriesBezierControl" + i].Points.AddXY(j, num5);
                }
            }
            else
            {
                ...
                {
                    num5 = (int)((a * k) + b);
                    _ = chartDeGamma.Series["SeriesBezierControl" + i].Points.AddXY(k, num5);
                }
            }
        }
    }
    ...
    private void chartDeGamma_MouseMove(object sender, MouseEventArgs e)
    {
        try
        {
            ...
            m_ptFeatureR[m_nFeatureIdxR] = m_nFeatureIdxR == 0
                ...
                : m_nFeatureIdxR == DeGammaNumber - 1
                    ? axisValuesFromMouse.Item2 < m_ptFeatureR[m_nFeatureIdxR - 1].Y
                                    ? new Point(m_ptFeatureR[m_nFeatureIdxR].X, m_ptFeatureR[m_nFeatureIdxR - 1].Y)
                                    : axisValuesFromMouse.Item2 > nYMax
                                        ? new Point(m_ptFeatureR[m_nFeatureIdxR].X, nYMax)
                                        : new Point(m_ptFeatureR[m_nFeatureIdxR].X, (int)axisValuesFromMouse.Item2)
                    : axisValuesFromMouse.Item2 > m_ptFeatureR[m_nFeatureIdxR + 1].Y
                                    ? new Point(m_ptFeatureR[m_nFeatureIdxR].X, m_ptFeatureR[m_nFeatureIdxR + 1].Y)
                                    : axisValuesFromMouse.Item2 < m_ptFeatureR[m_nFeatureIdxR - 1].Y
            ...
            else if (m_nFeatureIdxR == m_ptFeatureR.Count - 1)
            {
                m_ptFeatureRControl[(m_nFeatureIdxR * 2) - 1] = m_ptFeatureR[m_nFeatureIdxR].Y + diff_left_control_y < m_ptFeatureR[m_nFeatureIdxR - 1].Y
                    ? new Point(m_ptFeatureR[m_nFeatureIdxR].X, m_ptFeatureR[m_nFeatureIdxR - 1].Y)
                    : m_ptFeatureR[m_nFeatureIdxR].Y + diff_left_control_y > nYMax
                        ? new Point(m_ptFeatureR[m_nFeatureIdxR].X, nYMax)
                        : new Point(m_ptFeatureR[m_nFeatureIdxR].X, m_ptFeatureR[m_nFeatureIdxR].Y + diff_left_control_y);
            }
            clearControlLine();
            ...
            for (int i = 0; i < m_ptFeatureR.Count; i++)
            {
                _ = chartDeGamma.Series["SeriesDeGammaFeature"].Points.AddXY(m_ptFeatureR[i].X, m_ptFeatureR[i].Y);
            }
        }
        catch
        {
    ...
    public void setRedDeGammaArray(long[] pDeGamma, int index)
    {
        ...
        {
            _ = chartDeGamma.Series["SeriesDeGamma"].Points.AddXY(num, pDeGamma[i]);
            num++;
            if (OpMode == 0)
            {
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Auto[i], pDeGamma[i]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Auto[i], pDeGamma[i]);
            }
            else
            {
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Manual[i], pDeGamma[i]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Manual[i], pDeGamma[i]);
            }
        }
        Series seiresPoints = chartDeGamma.Series["SeriesDeGammaFeature"];
        Series seiresLines = chartDeGamma.Series["SeriesDeGamma"];
    ...
    private void DeGammabtnLoad_Click(object sender, EventArgs e)
    {
        ...
        if (OpMode == 0)
        {
            ...
            {
                xnode_Auto[i] = i % DeGammaNumber != 0 ? xnode_Auto[i - 1] + 128 : 0;
            }
        }
        else
        {
            ...
            {
                xnode_Manual[j] = j % DeGammaNumber != 0 ? xnode_Manual[j - 1] + 128 : 0;
            }
        }
        for (int k = 0; k < 33; k++)
        {
            m_ptFeatureR.Add(new Point(k, array[k]));
            _ = chartDeGamma.Series["SeriesDeGamma"].Points.AddXY(k, array[k]);
            _ = chartDeGamma.Series["SeriesDeGammaFeature"].Points.AddXY(k, array[k]);
            if (OpMode == 0)
            {
                ynode_Auto[(Index * DeGammaNumber) + k] = array[k];
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Auto[(Index * DeGammaNumber) + k], array[k]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Auto[(Index * DeGammaNumber) + k], array[k]);
            }
            else
            {
                ynode_Manual[k] = array[k];
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Manual[k], array[k]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Manual[k], array[k]);
            }
        }
        UpdateLineControl();
        SaveDeGammaValue_temp();
    ...
    private void DeGammabtnSave_Click(object sender, EventArgs e)
    {
        ...
        stream.Close();
        _ = MessageBox.Show("Save GlobalToneDeGamma Data OK");
    }
    ...
    private void RemoveFeaturePoint(Series pCurve, Series SeriesDeGammaFeature, List<Point> pFeaturePoint, Point ptSelect, ref bool bFindPoint)
    {
        ...
        for (int i = 0; i < pFeaturePoint.Count; i++)
        {
            if (pFeaturePoint[i].X > ptSelect.X - num && pFeaturePoint[i].X < ptSelect.X + num && pFeaturePoint[i].Y > ptSelect.Y - num && pFeaturePoint[i].Y < ptSelect.Y + num)
            {
                ...
                for (int j = 0; j < pFeaturePoint.Count; j++)
                {
                    _ = SeriesDeGammaFeature.Points.AddXY(pFeaturePoint[j].X, pFeaturePoint[j].Y);
                }
                break;
            }
        }
    ...
    {
        GetDeGammaWithOB(1024m, 1024m, 0m, out Point[] pDeGammaArray);
        if (pDeGammaArray != null)
        {
            ...
            {
                _ = chartDeGamma.Series[strDeGammaName].Points.AddXY(num, pDeGammaArray[num].Y);
            }
    ...
    private void initialDeGammavalue(Series seiresCurve, int index, Series seiresPoint, long[] DeGammavalue, List<Point> ptFeature, List<Point> ptFeatureControl)
    {
        ...
        for (int i = 33 * index; i < 33 * (index + 1); i++)
        {
            _ = seiresCurve.Points.AddXY(num, DeGammavalue[i]);
            num++;
        }
    ...
    private void initialDeGammaValue()
    {
        ...
        for (int i = 0; i < DeGammaNumber * NumberOfGroup; i++)
        {
            xnode_Auto[i] = i % DeGammaNumber != 0 ? xnode_Auto[i - 1] + m_nDeGammaInterval : 0;
        }
        ...
        {
            xnode_Manual[m] = m % DeGammaNumber != 0 ? xnode_Manual[m - 1] + m_nDeGammaInterval : 0;
        }
    ...
    public void ReadPage()
    {
        ...
        if (OpMode == 1)
        {
            for (int m = 0; m < 33; m++)
            {
                ...
                _ = chartDeGamma.Series["SeriesDeGamma"].Points.AddXY(m, ynode_Manual[m]);
                _ = chartDeGamma.Series["SeriesDeGammaFeature"].Points.AddXY(m, ynode_Manual[m]);
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Manual[m], ynode_Manual[m]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Manual[m], ynode_Manual[m]);
            }
        }
        else
        {
            for (int n = 0; n < 33; n++)
            {
                ...
                _ = chartDeGamma.Series["SeriesDeGamma"].Points.AddXY(n, ynode_Auto[(Index * DeGammaNumber) + n]);
                _ = chartDeGamma.Series["SeriesDeGammaFeature"].Points.AddXY(n, ynode_Auto[(Index * DeGammaNumber) + n]);
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Auto[(Index * DeGammaNumber) + n], ynode_Auto[(Index * DeGammaNumber) + n]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Auto[(Index * DeGammaNumber) + n], ynode_Auto[(Index * DeGammaNumber) + n]);
            }
        }
    }
    ...
    private void RenewSimulation()
    {
        ...
        for (int j = 0; j < DeGammaNumber; j++)
        {
            xnode_Manual[j] = j % DeGammaNumber != 0 ? xnode_Manual[j - 1] + m_nDeGammaInterval : 0;
        }
        for (int k = 0; k < NumberOfGroup; k++)
        {
            for (int l = 0; l < DeGammaNumber; l++)
        ...
        if (OpMode == 1)
        {
            for (int n = 0; n < 33; n++)
            {
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Manual[n], ynode_Manual[n]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Manual[n], ynode_Manual[n]);
            }
        }
        else
        {
            ...
            {
                _ = chartSimulationDeGamma.Series["SimulationDeGamma"].Points.AddXY(xnode_Auto[(Index * DeGammaNumber) + num], ynode_Auto[(Index * DeGammaNumber) + num]);
                _ = chartSimulationDeGamma.Series["SimulationDeGammaFeature"].Points.AddXY(xnode_Auto[(Index * DeGammaNumber) + num], ynode_Auto[(Index * DeGammaNumber) + num]);
            }
        }
    }
    ...
    private void InitializeComponent()
    {
        ...
        chartDeGamma.ChartAreas.Add(chartArea);
        chartDeGamma.Cursor = Cursors.Cross;
        legend.Name = "Legend1";
        chartDeGamma.Legends.Add(legend);
        chartDeGamma.Location = new Point(12, 12);
        ...
        DeGammacomboBoxIndex.SelectedIndex = 0;
        ...
        DeGammalabelPosition.Location = new Point(10, 362);
        DeGammalabelPosition.Name = "DeGammalabelPosition";
        DeGammalabelPosition.Size = new Size(101, 12);
        DeGammalabelPosition.TabIndex = 162;
        DeGammalabelPosition.Text = "Position:Out of chart";
        chartArea2.Name = "ChartArea1";
        chartSimulationDeGamma.ChartAreas.Add(chartArea2);
        chartSimulationDeGamma.Cursor = Cursors.Cross;
        legend2.Name = "Legend1";
        chartSimulationDeGamma.Legends.Add(legend2);
        chartSimulationDeGamma.Location = new Point(12, 377);
        ...
        series3.Name = "SimulationDeGamma";
        ...
        series4.Name = "SimulationDeGammaFeature";
        series4.YValuesPerPoint = 2;
        chartSimulationDeGamma.Series.Add(series3);
        chartSimulationDeGamma.Series.Add(series4);
        chartSimulationDeGamma.Size = new Size(520, 244);
        chartSimulationDeGamma.TabIndex = 163;
