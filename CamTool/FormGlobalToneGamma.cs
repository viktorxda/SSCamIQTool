...
public class FormGlobalToneGamma : Form
{
    ...
    private readonly int m_nGammaLength = 33;

    private readonly int m_nGammaInterval = 2048;
    ...
    private readonly int GammaNumber = 33;
    ...
    private static readonly int[] RecordGlobalToneGammaIndex_Auto = new int[528];

    public static long[] gammainitialvalueR_Manual;

    private static readonly int[] RecordGlobalToneGammaIndex_Manual = new int[33];
    ...
    private void GammacheckBoxEnable_CheckedChanged(object sender, EventArgs e)
    {
        ...
        chartGamma.Enabled = GammaEnable != 0;
    }
    ...
    public FormGlobalToneGamma(GuiGroup group)
    {
        InitializeComponent();
        gammaGroup = group;
    ...
    public void InitChart()
    {
        ...
        if (pGammaArray != null)
        {
            ...
            {
                _ = chartGamma.Series["SeriesGamma"].Points.AddXY(i, pGammaArray[i].Y);
            }
    ...
    public void InitBezierControl(Series seiresRLine, Series seiresRPoints, List<Point> m_ptFeatureR, List<Point> m_ptFeatureRControl)
    {
        ...
        for (int j = 0; j < m_ptFeatureRControl.Count; j++)
        {
            ...
            if (j % 2 == 0)
            {
                ...
                {
                    num27 = (int)((a * k) + b);
                    _ = chartGamma.Series["SeriesBezierControl" + j].Points.AddXY(k, num27);
                }
            }
            else
            {
                ...
                {
                    num27 = (int)((a * l) + b);
                    _ = chartGamma.Series["SeriesBezierControl" + j].Points.AddXY(l, num27);
                }
            }
        }
    }
    ...
    private void chartGamma_MouseMove(object sender, MouseEventArgs e)
    {
        try
        {
            ...
            for (int i = 0; i < m_ptFeatureR.Count; i++)
            {
                _ = chartGamma.Series["SeriesGammaFeature"].Points.AddXY(m_ptFeatureR[i].X, m_ptFeatureR[i].Y);
            }
        }
        catch
        {
    ...
    public void setRedGammaArray(long[] pGamma, int index)
    {
        ...
        {
            _ = chartGamma.Series["SeriesGamma"].Points.AddXY(num, pGamma[i]);
            num++;
            if (OpMode == 0)
            {
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Auto[i], pGamma[i]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Auto[i], pGamma[i]);
            }
            else
            {
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Manual[i], pGamma[i]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Manual[i], pGamma[i]);
            }
        }
        Series seiresPoints = chartGamma.Series["SeriesGammaFeature"];
        Series seiresLines = chartGamma.Series["SeriesGamma"];
    ...
    private void GammabtnLoad_Click(object sender, EventArgs e)
    {
        ...
        if (OpMode == 0)
        {
            ...
            {
                xnode_Auto[i] = i % GammaNumber != 0 ? xnode_Auto[i - 1] + 2048 : 0;
            }
        }
        else
        {
            ...
            {
                xnode_Manual[j] = j % GammaNumber != 0 ? xnode_Manual[j - 1] + 2048 : 0;
            }
        }
        for (int k = 0; k < 33; k++)
        {
            m_ptFeatureR.Add(new Point(k, array[k]));
            _ = chartGamma.Series["SeriesGamma"].Points.AddXY(k, array[k]);
            _ = chartGamma.Series["SeriesGammaFeature"].Points.AddXY(k, array[k]);
            if (OpMode == 0)
            {
                ynode_Auto[(Index * GammaNumber) + k] = array[k];
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Auto[(Index * GammaNumber) + k], array[k]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Auto[(Index * GammaNumber) + k], array[k]);
            }
            else
            {
                ynode_Manual[k] = array[k];
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Manual[k], array[k]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Manual[k], array[k]);
            }
        }
        UpdateLineControl();
        SaveGammaValue_temp();
    ...
    private void GammabtnSave_Click(object sender, EventArgs e)
    {
        ...
        stream.Close();
        _ = MessageBox.Show("Save GlobalToneGamma Data OK");
    }
    ...
    private void RemoveFeaturePoint(Series pCurve, Series SeriesGammaFeature, List<Point> pFeaturePoint, Point ptSelect, ref bool bFindPoint)
    {
        ...
        for (int i = 0; i < pFeaturePoint.Count; i++)
        {
            if (pFeaturePoint[i].X > ptSelect.X - num && pFeaturePoint[i].X < ptSelect.X + num && pFeaturePoint[i].Y > ptSelect.Y - num && pFeaturePoint[i].Y < ptSelect.Y + num)
            {
                ...
                for (int j = 0; j < pFeaturePoint.Count; j++)
                {
                    _ = SeriesGammaFeature.Points.AddXY(pFeaturePoint[j].X, pFeaturePoint[j].Y);
                }
                break;
            }
        }
    ...
    private void initialGammaValue()
    {
        ...
        for (int i = 0; i < GammaNumber * NumberOfGroup; i++)
        {
            xnode_Auto[i] = i % GammaNumber != 0 ? xnode_Auto[i - 1] + m_nGammaInterval : 0;
        }
        ...
        {
            xnode_Manual[m] = m % GammaNumber != 0 ? xnode_Manual[m - 1] + m_nGammaInterval : 0;
        }
    ...
    public void ReadPage()
    {
        ...
        if (OpMode == 1)
        {
            for (int m = 0; m < 33; m++)
            {
                ...
                _ = chartGamma.Series["SeriesGamma"].Points.AddXY(m, ynode_Manual[m]);
                _ = chartGamma.Series["SeriesGammaFeature"].Points.AddXY(m, ynode_Manual[m]);
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Manual[m], ynode_Manual[m]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Manual[m], ynode_Manual[m]);
            }
        }
        else
        {
            for (int n = 0; n < 33; n++)
            {
                ...
                _ = chartGamma.Series["SeriesGamma"].Points.AddXY(n, ynode_Auto[(Index * GammaNumber) + n]);
                _ = chartGamma.Series["SeriesGammaFeature"].Points.AddXY(n, ynode_Auto[(Index * GammaNumber) + n]);
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Auto[(Index * GammaNumber) + n], ynode_Auto[(Index * GammaNumber) + n]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Auto[(Index * GammaNumber) + n], ynode_Auto[(Index * GammaNumber) + n]);
            }
        }
    }
    ...
    private void RenewSimulation()
    {
        ...
        for (int j = 0; j < GammaNumber; j++)
        {
            xnode_Manual[j] = j % GammaNumber != 0 ? xnode_Manual[j - 1] + m_nGammaInterval : 0;
        }
        for (int k = 0; k < NumberOfGroup; k++)
        {
            for (int l = 0; l < GammaNumber; l++)
        ...
        if (OpMode == 1)
        {
            for (int n = 0; n < 33; n++)
            {
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Manual[n], ynode_Manual[n]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Manual[n], ynode_Manual[n]);
            }
        }
        else
        {
            ...
            {
                _ = chartSimulationGamma.Series["SimulationGamma"].Points.AddXY(xnode_Auto[(Index * GammaNumber) + num], ynode_Auto[(Index * GammaNumber) + num]);
                _ = chartSimulationGamma.Series["SimulationGammaFeature"].Points.AddXY(xnode_Auto[(Index * GammaNumber) + num], ynode_Auto[(Index * GammaNumber) + num]);
            }
        }
    }
    ...
    {
        _ = GammacomboBoxIndex.Focus();
    }

    private void chartGamma_SelectionRangeChanging(object sender, CursorEventArgs e)
    {
        e.NewSelectionStart = e.NewSelectionStart;
        e.NewSelectionEnd = e.NewSelectionEnd;
    }
    ...
    private void InitializeComponent()
    {
        ...
        chartGamma.Location = new Point(12, 12);
        ...
        GammacomboBoxIndex.SelectedIndex = 0;
        ...
        GammalabelPosition.Location = new Point(10, 362);
        GammalabelPosition.Name = "GammalabelPosition";
        GammalabelPosition.Size = new Size(101, 12);
        GammalabelPosition.TabIndex = 161;
        GammalabelPosition.Text = "Position:Out of chart";
        chartArea2.Name = "ChartArea1";
        chartSimulationGamma.ChartAreas.Add(chartArea2);
        chartSimulationGamma.Cursor = Cursors.Cross;
        legend2.Name = "Legend1";
        chartSimulationGamma.Legends.Add(legend2);
        chartSimulationGamma.Location = new Point(12, 377);
        ...
        series4.Name = "SimulationGammaFeature";
        series4.YValuesPerPoint = 2;
        chartSimulationGamma.Series.Add(series3);
        chartSimulationGamma.Series.Add(series4);
        chartSimulationGamma.Size = new Size(520, 244);
        chartSimulationGamma.TabIndex = 162;
