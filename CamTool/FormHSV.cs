...
public class FormHSV : Form
{
    ...
    private readonly GuiGroup group;

    private readonly IQComm comm;

    private readonly Series seriesLine;
    ...
    public FormHSV(GuiGroup group, GuiItem item, IQComm comm, bool bAW)
    {
        ...
        if (item.Text.EndsWith("HueAngleShiftLut") || item.Text.EndsWith("Hue") || item.Text.Contains("HueLut"))
        {
            ...
            {
                _ = MessageBox.Show("/Icon/hue.png not found");
            }
        }
        ...
        {
            _ = MessageBox.Show("/Icon/sat.png not found");
        }
        InitChart(item);
        Text = item.Text;
        this.group = group;
    ...
    private void chartHSV_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        featurePoint[nSelectIdx] = axisValuesFromMouse.Item2 < YMin
            ? new Point(featurePoint[nSelectIdx].X, YMin)
            : axisValuesFromMouse.Item2 > YMax
                ? new Point(featurePoint[nSelectIdx].X, YMax)
                : new Point(featurePoint[nSelectIdx].X, (int)Math.Round(axisValuesFromMouse.Item2));
        seriesPoint.Points.Clear();
        foreach (Point item in featurePoint)
        {
            _ = seriesPoint.Points.AddXY(item.X, item.Y);
        }
    ...
    private void UpdateSeriesPoint()
    {
        ...
        {
            _ = seriesPoint.Points.AddXY(i, item.DataValue[(selectedIndex * item.XSize) + i]);
            Point point = new(i, (int)item.DataValue[(selectedIndex * item.XSize) + i]);
