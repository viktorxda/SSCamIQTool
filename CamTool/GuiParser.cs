...
public class GuiParser
{
    ...
    private readonly XmlDocument m_xmlDoc = new();

    private readonly XmlDocument m_xmlCacheDoc = new();

    private readonly List<GuiPage> m_GuiPageCacheSavedOne = new();

    private readonly List<GuiPage> m_GuiPageCacheSavedTwo = new();

    private readonly List<int> m_CuiPageCurrentSaved = new();

    private readonly List<GuiPage> m_GuiPageArray = new();

    private readonly List<GuiPage> m_GuiPageSaved = new();

    private readonly List<GuiPage> m_GuiPageRollback = new();

    private List<List<Control>>[] m_PageControlArray;

    private readonly ActionList actionList = new();
    ...
    public List<GuiPage> m_CvtSrcGuiPageArray = new();

    private readonly List<List<Control>> m_CvtSrcPageControlArray = new();
    ...
    public List<GuiPage> m_CvtDesGuiPageArray = new();

    private readonly List<List<Control>> m_CvtDesPageControlArray = new();
    ...
    public bool LoadCvtSrc(string file)
    {
        if (Path.GetExtension(file).Equals(".bin"))
        {
            ...
            if (File.Exists(text))
            {
                _ = InitCvtSrcGUI(text);
                LoadCvtSrcBinFile(file);
                return true;
            }
            _ = MessageBox.Show(text + " does not exist.");
            return false;
        }
        if (Path.GetExtension(file).Equals(".xml"))
        {
            ...
            _ = MessageBox.Show(file + " cannot be load.");
            return false;
        }
        return false;
    }
    ...
    public void copyCvtSrcToCvtDes()
    {
        ...
        {
            _ = MessageBox.Show("Warning! API of Souce Bin File Not Exist.", "Connection", MessageBoxButtons.OK, MessageBoxIcon.Hand);
            return;
        }
        if (m_CvtDesGuiPageArray.Count == 0)
        {
            _ = MessageBox.Show("Warning! API of Destination Bin File Not Exist.", "Connection", MessageBoxButtons.OK, MessageBoxIcon.Hand);
            return;
        }
    ...
    public bool InitGUI(string strFileName)
    {
        ...
        if (GetGuiPageListFromXml(strFileName, m_GuiPageArray).Equals(""))
        {
            ...
            {
                _ = m_treePageItem.Nodes[0].Nodes.Add(m_GuiPageArray[i].Name);
            }
    ...
    public bool InitFileGUI(int FileIdx, string strFileName)
    {
        ...
        _ = m_FiletreePageItem.Nodes[0].Nodes.Add(strFileName);
    ...
    public bool InitCvtSrcGUI(string strFileName)
    {
        ...
        if (GetGuiPageListFromXml(strFileName, m_CvtSrcGuiPageArray).Equals(""))
        {
            ...
            {
                _ = m_CvtSrctreePageItem.Nodes[0].Nodes.Add(m_CvtSrcGuiPageArray[i].Name);
            }
    ...
    public bool InitCvtDesGUI(string strFileName)
    {
        ...
        if (GetGuiPageListFromXml(strFileName, m_CvtDesGuiPageArray).Equals(""))
        {
            ...
            for (int i = 0; i < m_CvtDesGuiPageArray.Count; i++)
            {
                _ = m_CvtDestreePageItem.Nodes[0].Nodes.Add(m_CvtDesGuiPageArray[i].Name);
            }
    ...
    private void CreateNormalPage(GuiPage sUIPage)
    {
        ...
        for (int j = 0; j < sUIPage.GroupList.Count; j++)
        {
            ...
            {
                RowCount = sUIPage.GroupList[j].ItemList.Count,
                ColumnCount = 2,
            ...
            {
                Checked = sUIPage.GroupList[j].InFile,
                Enabled = true,
                AutoSize = true,
            ...
            while (num3 < sUIPage.GroupList[j].ItemList.Count)
            {
                ...
                if (xSize * ySize > 1 && (sUIPage.GroupList[j].ItemList[num3].GuiType == "DataGrid" || sUIPage.GroupList[j].ItemList[num3].GuiType == "DataShading" || sUIPage.GroupList[j].ItemList[num3].GuiType == "DataHSV" || sUIPage.GroupList[j].ItemList[num3].GuiType == "DataAE" || sUIPage.GroupList[j].ItemList[num3].GuiType == "DataLine" || sUIPage.GroupList[j].ItemList[num3].GuiType == "DataLut"))
                {
                    Label label = new()
                    {
                        Text = sUIPage.GroupList[j].ItemList[num3].Text,
                        ForeColor = Color.Blue,
                        BackColor = Color.Silver,
                        Font = new Font("Times New Roman", 9f, FontStyle.Regular),
                        Width = guiParamNameWidth,
                        Dock = DockStyle.Fill,
                        Margin = new Padding(0, 0, 0, 0),
                        TextAlign = ContentAlignment.MiddleRight,
                        Tag = sUIPage.GroupList[j].ItemList[num3].Tag
                    };
                    ...
                    Button button2 = new()
                    {
                        Text = sUIPage.GroupList[j].Action == API_ACTION.R ? "Show Table" : "Edit Table",
                        Tag = sUIPage.GroupList[j].ItemList[num3].Tag,
                        Width = guiButtonWidth
                    };
                ...
                else if (sUIPage.GroupList[j].ItemList[num3].GuiType.Equals("CheckBox"))
                {
                    ...
                    checkBox2.Checked = sUIPage.GroupList[j].ItemList[num3].DataValue[0] > 0;
                }
                else if (sUIPage.GroupList[j].ItemList[num3].GuiType.StartsWith("RadioButton"))
                {
                    ...
                    {
                        Name = "radioButton_" + sUIPage.GroupList[j].Name.Replace(" ", "") + "_" + sUIPage.GroupList[j].ItemList[num3].Tag,
                        Tag = sUIPage.GroupList[j].ItemList[num3].Tag,
                        Text = "Disable",
                        AutoSize = true,
                        Left = 10
                    ...
                    RadioButton radioButton2 = new()
                    {
                        ...
                        Text = "Enable",
                        AutoSize = true,
                        Left = radioButton.Size.Width + 20
                    };
                ...
                else if (sUIPage.GroupList[j].ItemList[num3].GuiType.StartsWith("ComboBox"))
                {
                    ...
                    if (!sUIPage.GroupList[j].ItemList[num3].Paramters.Equals(""))
                    {
                        ...
                        {
                            _ = comboBox.Items.Add(array2[num5]);
                        }
                    }
                    else
                    {
                ...
                else if (sUIPage.GroupList[j].ItemList[num3].GuiType.Equals("NumScroll"))
                {
                    ...
                    Panel panel5 = new()
                    {
                        Width = guiNumScrollWidth + num,
                        Height = 25
                    };
                    NumericUpDown numericUpDown = new()
                    {
                        Width = guiNumScrollWidth,
                        Maximum = sUIPage.GroupList[j].ItemList[num3].MaxValue,
                        Minimum = sUIPage.GroupList[j].ItemList[num3].MinValue,
                        Tag = sUIPage.GroupList[j].ItemList[num3].Tag
                    };
                    ...
                    SstarHScrollBar sstarHScrollBar = new()
                    {
                        Tag = sUIPage.GroupList[j].ItemList[num3].Tag,
                        Maximum = (int)sUIPage.GroupList[j].ItemList[num3].MaxValue,
                        Minimum = (int)sUIPage.GroupList[j].ItemList[num3].MinValue,
                        LargeChange = 1
                    };
                    if (sUIPage.GroupList[j].Action == API_ACTION.R)
                    {
    ...
    private void CreateCvtSrcNormalPage(GuiPage sUIPage)
    {
        ...
        for (int i = 0; i < m_pCvtSrcGroupBoxArray.Length; i++)
        {
            m_pCvtSrcGroupBoxArray[i] = new GroupBox
            {
                Width = width,
                Height = height,
                Text = sUIPage.GroupList[i].Name,
                Name = "groupBox_" + sUIPage.GroupList[i].Name.Replace(" ", "_"),
                Font = new Font("Times New Roman", 10f, FontStyle.Regular),
                BackColor = Color.LightGray
            };
            ...
            TableLayoutPanel tableLayoutPanel = new()
            {
                RowCount = sUIPage.GroupList[i].ItemList.Count,
                ColumnCount = 2,
                CellBorderStyle = TableLayoutPanelCellBorderStyle.Single
            };
            int num3 = 0;
            ...
            while (num3 < sUIPage.GroupList[i].ItemList.Count)
            {
                ...
                if (xSize * ySize > 1 && (sUIPage.GroupList[i].ItemList[num3].GuiType == "DataGrid" || sUIPage.GroupList[i].ItemList[num3].GuiType == "DataShading" || sUIPage.GroupList[i].ItemList[num3].GuiType == "DataHSV"))
                {
                    Label label = new()
                    {
                        Name = "Label_" + sUIPage.GroupList[i].Name.Replace(" ", "_") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Text = sUIPage.GroupList[i].ItemList[num3].Text,
                        ForeColor = Color.Blue,
                        BackColor = Color.Silver,
                        Font = new Font("Times New Roman", 9f, FontStyle.Regular),
                        Width = guiParamNameWidth,
                        ...
                        Margin = new Padding(0, 0, 0, 0),
                        TextAlign = ContentAlignment.MiddleRight,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag
                    };
                    ...
                    Button button = new()
                    {
                        Text = sUIPage.GroupList[i].Action == API_ACTION.R ? "Show Table" : "Edit Table",
                        Name = "button_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag,
                        Width = guiButtonWidth
                    };
                ...
                else if (sUIPage.GroupList[i].ItemList[num3].GuiType.Equals("CheckBox"))
                {
                    ...
                    CheckBox checkBox = new()
                    {
                        Name = "checkBox_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Dock = DockStyle.Fill,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag
                    };
                    ...
                    checkBox.Checked = sUIPage.GroupList[i].ItemList[num3].DataValue[0] > 0;
                }
                else if (sUIPage.GroupList[i].ItemList[num3].GuiType.StartsWith("RadioButton"))
                {
                    ...
                    RadioButton radioButton = new()
                    {
                        Name = "radioButton_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag,
                        Text = "Disable",
                        AutoSize = true,
                        Left = 10
                ...
                else if (sUIPage.GroupList[i].ItemList[num3].GuiType.StartsWith("ComboBox"))
                {
                    ...
                    ComboBox comboBox = new()
                    {
                        Name = "comboBox_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag,
                        Width = guiButtonWidth2,
                        DropDownStyle = ComboBoxStyle.DropDownList
                    };
                    if (!sUIPage.GroupList[i].ItemList[num3].Paramters.Equals(""))
                    {
                        ...
                        {
                            _ = comboBox.Items.Add(array[num5]);
                        }
                    }
                    else
                    {
                ...
                else if (sUIPage.GroupList[i].ItemList[num3].GuiType.StartsWith("TextBox"))
                {
                    ...
                    TextBox textBox = new()
                    {
                        Name = "textBox_" + sUIPage.GroupList[i].Name.Replace(" ", "_") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag
                    };
                    if (sUIPage.GroupList[i].Action == API_ACTION.R)
                    {
                        textBox.ReadOnly = true;
                ...
                else if (sUIPage.GroupList[i].ItemList[num3].GuiType.Equals("NumScroll"))
                {
                    Label label6 = new()
                    {
                        Name = "Label_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Text = sUIPage.GroupList[i].ItemList[num3].Text,
                        ForeColor = Color.Blue,
                        BackColor = Color.Silver,
                        Font = new Font("Times New Roman", 9f, FontStyle.Regular),
                    ...
                    NumericUpDown numericUpDown = new()
                    {
                        Name = "numUpDn_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Width = guiNumScrollWidth,
                        Maximum = sUIPage.GroupList[i].ItemList[num3].MaxValue,
                        Minimum = sUIPage.GroupList[i].ItemList[num3].MinValue,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag
                    };
                    if (sUIPage.GroupList[i].Action == API_ACTION.R)
                    {
                    ...
                    {
                        Name = "hScrollBar_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag,
                        Maximum = (int)sUIPage.GroupList[i].ItemList[num3].MaxValue,
                        Minimum = (int)sUIPage.GroupList[i].ItemList[num3].MinValue,
                        LargeChange = 1
                    };
                    if (sUIPage.GroupList[i].Action == API_ACTION.R)
                    {
                ...
                else if (sUIPage.GroupList[i].ItemList[num3].GuiType.Equals("CalcButton"))
                {
                    ...
                    {
                        Text = sUIPage.GroupList[i].ItemList[num3].Text,
                        Name = "calcButton_" + sUIPage.GroupList[i].Name.Replace(" ", "") + "_" + sUIPage.GroupList[i].ItemList[num3].Tag,
                        Tag = sUIPage.GroupList[i].ItemList[num3].Tag
                    };
    ...
    private void CreateRGBGammaPage(GuiPage sUIPage)
    {
        m_ContentTitle.Text = sUIPage.Name;
        m_ContentPanel.Controls.Clear();
        RGBGamma rGBGamma = new(sUIPage.GroupList[0]);
    ...
    private void CreateYUVGammaPage(GuiPage sUIPage)
    {
        m_ContentTitle.Text = sUIPage.Name;
        m_ContentPanel.Controls.Clear();
        YUVGamma yUVGamma = new(sUIPage.GroupList[0]);
    ...
    private void CreateWDRCurvePage(GuiPage sUIPage)
    {
        m_ContentTitle.Text = sUIPage.Name;
        m_ContentPanel.Controls.Clear();
        WDRCurve wDRCurve = new(sUIPage.GroupList[0]);
    ...
    private void CreateWDRCurve_v2Page(GuiPage sUIPage)
    {
        m_ContentTitle.Text = sUIPage.Name;
        m_ContentPanel.Controls.Clear();
        WDRCurve_v2 wDRCurve_v = new(sUIPage.GroupList[0], m_DbgParser);
    ...
    private void checkBox_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (!m_bIsCreatingGui)
        {
            ...
            if (AutoWrite && !m_noAutoWrite && m_DbgParser.IsConnected() && guiGroup.Action == API_ACTION.RW)
            {
    ...
    private void textValueItem_NumLostFocus(object sender, EventArgs e)
    {
        ...
        if (!textBox.ReadOnly)
        {
            _ = FindGuiItemByItemName(m_GuiPageArray[m_CurrPageIndex], text);
            try
            {
            ...
            {
                _ = MessageBox.Show(ex.Message, "Status", MessageBoxButtons.OK);
                return;
            }
    ...
    private void radioButton0_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (!m_bIsCreatingGui)
        {
            ...
            SetBackStepMode(mode: true);
            SetNextStepMode(mode: false);
            if (AutoWrite && !m_noAutoWrite && m_DbgParser.IsConnected() && (guiGroup.Action == API_ACTION.RW || guiGroup.Action == API_ACTION.W))
            {
    ...
    private void comboBox_show_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        GuiGroup guiGroup = GetGuiGroupByItemName(text);
        if (!m_bIsCreatingGui)
        {
            GuiItem guiItem = guiGroup.FindItemByName(text);
    ...
    private void textValueItem_TextChanged(object sender, EventArgs e)
    {
        ...
        if (!m_bIsCreatingGui)
        {
            _ = guiGroup.FindItemByName(textBox.Tag.ToString());
    ...
    private void textValueItem_NumTextChanged(object sender, EventArgs e)
    {
        ...
        TextBox textBox = (TextBox)sender;
        long[] array = new long[1];
        GuiGroup guiGroup = GetGuiGroupByItemName(textBox.Tag.ToString());
        if (!m_bIsCreatingGui)
        {
            _ = guiGroup.FindItemByName(textBox.Tag.ToString());
            try
    ...
    private void CvtSrctextValueItem_NumTextChanged(object sender, EventArgs e)
    {
        TextBox textBox = (TextBox)sender;
        long[] array = new long[1];
        GuiGroup guiGroup = GetCvtSrcGuiGroupByItemName(textBox.Tag.ToString());
        if (!m_bIsCreatingGui)
        {
            _ = guiGroup.FindItemByName(textBox.Tag.ToString());
            try
    ...
    private void vComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        long[] array = new long[1];
        string text = comboBox.Tag.ToString();
        GuiPage guiPage = m_GuiPageArray[m_CurrPageIndex];
        GuiGroup guiGroup = GetGuiGroupByItemName(text);
        if (!m_bIsCreatingGui)
        {
            GuiItem guiItem = guiGroup.FindItemByName(text);
    ...
    private void ByPassTask(object objList)
    {
        ...
        if (text != "")
        {
            _ = m_ContentPanel.Invoke(new ShowMessageHandler(ShowMessage), text, "");
        }
    }
    ...
    private void SendPageTask(object objList)
    {
        ...
        _ = m_ContentPanel.Invoke(new ShowMessageHandler(ShowMessage), text2, text3);
        CheckConnection();
    }
    ...
    private void ReadPageTask(object objList)
    {
        ...
        if (text.Equals(""))
        {
            ...
            _ = m_ContentPanel.Invoke(new UpdateGUIHandler(UpdateGUI));
    ...
    private void SendAllTask(object objList)
    {
        ...
        _ = m_ContentPanel.Invoke(new ShowMessageHandler(ShowMessage), text3, text4);
        CheckConnection();
    }
    ...
    private void ReadAllTask(object obj)
    {
        ...
        for (int i = 0; i < m_GuiPageArray.Count; i++)
        {
            ...
            _ = m_ContentPanel.Invoke(new UpdateTransProgressByValueHandler(TransProgressByValue), i * 100 / m_GuiPageArray.Count, num3);
            if (text.Equals(""))
            {
        ...
        _ = m_ContentPanel.Invoke(new UpdateGUIHandler(UpdateGUI));
        _ = m_ContentPanel.Invoke(new UpdateTransFinishHandler(TransFinish), text2);
        if (text2 != "")
        {
    ...
    private void CalculateApiTask(object obj)
    {
        ...
        else
        {
            ...
            _ = m_ContentPanel.Invoke(new UpdateGUIHandler(UpdateGUI));
            _ = m_ContentPanel.Invoke(new UpdateTransFinishHandler(TransFinish), text);
            if (text != "")
            {
    ...
    private void CaliApiTask(object obj)
    {
        ...
        else
        {
            ...
            {
                _ = guiGroup.ReadCaliAWBGroup(m_DbgParser, FILE_TRANSFER_ID.CALIBRATION_DATA);
                _ = m_ContentPanel.Invoke(new UpdateGUIHandler(UpdateGUI));
            }
            if (guiGroup.Name is "CaliDPC" or "CaliSDC")
            {
                _ = guiGroup.ReadCaliDPCGroup(m_DbgParser, FILE_TRANSFER_ID.CALIBRATION_DATA);
                _ = m_ContentPanel.Invoke(new UpdateGUIHandler(UpdateGUI));
            }
            _ = m_ContentPanel.Invoke(new ShowMessageHandler(ShowMessage), text2, text3);
        }
    ...
    {
        _ = ((GuiGroup)obj).WriteGroup(m_DbgParser);
        CheckConnection();
    }
    ...
    private void ShadingButton_Click(object sender, EventArgs e)
    {
        ...
        FormShading formShading = new(GetGuiGroupByItemName(text), m_DbgParser, text);
        formShading.Show();
        _ = formShading.Focus();
    }
    ...
    {
        string text = ((Button)sender).Tag.ToString();
        FormShading formShading = new(GetCvtSrcGuiGroupByItemName(text), m_DbgParser, text);
        formShading.Show();
        _ = formShading.Focus();
    }
    ...
    {
        string text = ((Button)sender).Tag.ToString();
        FormShading formShading = new(GetCvtDesGuiGroupByItemName(text), m_DbgParser, text);
        formShading.Show();
        _ = formShading.Focus();
    }
    ...
    private void HSVButton_Click(object sender, EventArgs e)
    {
        ...
        GuiItem guiItem = guiGroupByItemName.FindItemByName(text);
        FormHSV formHSV = new(guiGroupByItemName, guiItem, m_DbgParser, AutoWrite);
        formHSV.Show();
        _ = formHSV.Focus();
    }
    ...
    private void CvtSrcHSVButton_Click(object sender, EventArgs e)
    {
        ...
        GuiItem guiItem = cvtSrcGuiGroupByItemName.FindItemByName(text);
        FormHSV formHSV = new(cvtSrcGuiGroupByItemName, guiItem, m_DbgParser, AutoWrite);
        formHSV.Show();
        _ = formHSV.Focus();
    }
    ...
    private void CvtDesHSVButton_Click(object sender, EventArgs e)
    {
        ...
        GuiItem guiItem = cvtDesGuiGroupByItemName.FindItemByName(text);
        FormHSV formHSV = new(cvtDesGuiGroupByItemName, guiItem, m_DbgParser, AutoWrite);
        formHSV.Show();
        _ = formHSV.Focus();
    }
    ...
    private void AEButton_Click(object sender, EventArgs e)
    {
        ...
        string text = ((Button)sender).Tag.ToString();
        FormAETable formAETable = new(GetGuiGroupByItemName(text), m_DbgParser, text, AutoWrite);
        _ = formAETable.ShowDialog();
        _ = formAETable.Focus();
    }
    ...
    private void gridButton_Click(object sender, EventArgs e)
    {
        ...
        string text = ((Button)sender).Tag.ToString();
        FormTable formTable = new(GetGuiGroupByItemName(text), m_DbgParser, text, AutoWrite);
        formTable.Show();
        _ = formTable.Focus();
    }
    ...
    {
        string text = ((Button)sender).Tag.ToString();
        FormTable formTable = new(GetCvtSrcGuiGroupByItemName(text), m_DbgParser, text, AutoWrite);
        formTable.Show();
        _ = formTable.Focus();
    }
    ...
    private void DataLineChart_Click(object sender, EventArgs e)
    {
        ...
        GuiGroup guiGroupByItemName = GetGuiGroupByItemName(text);
        GuiItem guiItem = guiGroupByItemName.FindItemByName(text);
        FormLineChart formLineChart = new(guiGroupByItemName, guiItem, m_DbgParser, AutoWrite);
        formLineChart.Show();
        _ = formLineChart.Focus();
    }
    ...
    private void LutChartButton_Click(object sender, EventArgs e)
    {
        ...
        GuiGroup guiGroupByItemName = GetGuiGroupByItemName(text);
        GuiItem guiItem = guiGroupByItemName.FindItemByName(text);
        FormLut formLut = new(guiGroupByItemName, guiItem, m_DbgParser, AutoWrite);
        formLut.Show();
        _ = formLut.Focus();
    }
    ...
    {
        string text = ((Button)sender).Tag.ToString();
        FormTable formTable = new(GetCvtDesGuiGroupByItemName(text), m_DbgParser, text, AutoWrite);
        formTable.Show();
        _ = formTable.Focus();
    }
    ...
    {
        string text = (sender as Control).Tag.ToString();
        GuiItem guiItem = GetCvtSrcGuiGroupByItemName(text).FindItemByName(text);
    ...
    {
        string text = (sender as Control).Tag.ToString();
        GuiItem guiItem = GetCvtDesGuiGroupByItemName(text).FindItemByName(text);
        m_CvtSrclabelItemInfo.Text = guiItem.Info;
    }

    private long ConvertStringToNum(string strValue)
    {
        return strValue.StartsWith("0x")
            ? Convert.ToInt64(strValue.Substring(2), 16)
            : strValue.StartsWith("-0x") ? -Convert.ToInt64(strValue.Substring(3), 16) : Convert.ToInt64(strValue);
    }
    ...
    private void GetXmlFileVer(string strFileName)
    {
        ...
        try
        {
            ...
            if (text5 == "1.0")
            {
                ...
                {
                    string text = childNode.GetAttribute("Author");
                    string text2 = childNode.GetAttribute("Version");
                    if (text == "ISP")
                    {
                        m_ispVersion = text2;
                    }
        ...
        {
            string text3 = "[Error] ParseXML: " + text4 + " error " + ex.Message;
            ShowMessage(text3, "");
        }
    }
                ...
                foreach (XmlElement childNode in xmlNode.ChildNodes)
                {
                    ...
                    foreach (XmlElement childNode2 in childNode.ChildNodes)
                    {
                        ...
                        foreach (XmlElement childNode3 in childNode2.ChildNodes)
                        {
                            ...
                            int[] index = Array.ConvertAll(text9.Split(','), int.Parse);
                            ...
                            int[] mode = !(text9 == "0") ? Array.ConvertAll(text9.Split('~'), int.Parse) : new int[1];
            ...
            else
            {
                ...
                foreach (XmlElement childNode5 in xmlNode.ChildNodes)
                {
                    ...
                    foreach (XmlElement childNode6 in childNode5.ChildNodes)
                    {
                        ...
                        int[] index2 = Array.ConvertAll(text9.Split(','), int.Parse);
                        text9 = childNode6.GetAttribute("Action");
                        API_ACTION action = (API_ACTION)Enum.Parse(typeof(API_ACTION), text9);
                        text9 = childNode6.GetAttribute("AutoMode");
                        int[] mode2 = !(text9 == "0") ? Array.ConvertAll(text9.Split('~'), int.Parse) : new int[1];
    ...
    public void ShowMessage(string msgBox, string msgText)
    {
        if (msgBox != "")
        {
            _ = MessageBox.Show(msgBox, "Status", MessageBoxButtons.OK);
        }
        if (msgText != "")
        {
    ...
    private void SetControlGroupValue(List<Control> controlList, GuiGroup group)
    {
        ...
        {
            _ = MessageBox.Show(group.Name + "." + group.ItemList[num].Text + "\n" + ex.Message, "Status", MessageBoxButtons.OK);
        }
    }
    ...
    {
        _ = m_GuiPageArray[m_CurrPageIndex].ReadUartPage(m_DbgParser);
        _ = m_ContentPanel.Invoke(new UpdateGUIHandler(UpdateGUI));
    }
    ...
    public void WriteUartPage()
    {
        ...
        _ = m_GuiPageArray[m_CurrPageIndex].WriteUartPage(m_DbgParser);
        string text = m_GuiPageArray[m_CurrPageIndex].getmv5length(m_DbgParser);
    ...
    public byte[] GetBinBytes(ChipID chipID, uint magicKey, ushort checksumVer)
    {
        ...
        ushort value3 = 0;
        uint value4 = 0u;
        int num3 = int.Parse(m_ispVersion.Split('.').ElementAt(0));
    ...
    public byte[] GetBinBytesXmlCRC(ChipID chipID, uint magicKey, uint videomagicKey, string zipPath, ushort checksumVer)
    {
        ...
        if (m_videoVersion != "")
        {
            int num3 = int.Parse(m_videoVersion.Split('.').ElementAt(0));
    ...
    public void GetBinFileVer(string filePath)
    {
        ...
        _ = BitConverter.ToInt32(binaryReader.ReadBytes(4), 0);
        _ = 1835336475;
    ...
    public void GetBinFileChip(string filePath)
    {
        ...
        if (num is 1835336481 or 1835336482 or 1835336484)
        {
    ...
    public void LoadCvtSrcBinFile(string filePath)
    {
        ...
        _ = BitConverter.ToUInt32(binaryReader.ReadBytes(4), 0);
        m_BinChecksumVer = BitConverter.ToUInt16(binaryReader.ReadBytes(2), 0);
        _ = BitConverter.ToUInt16(binaryReader.ReadBytes(2), 0);
        _ = BitConverter.ToUInt32(binaryReader.ReadBytes(4), 0);
    ...
    public int LoadCvtDesBinXmlFile(string filePath)
    {
        ...
        if ((int)binaryReader.BaseStream.Length > num + num2)
        {
            try
            {
                ...
                BinaryWriter binaryWriter = new(File.Open(text2, FileMode.Create));
                binaryWriter.Write(buffer2);
                binaryWriter.Close();
                DecompressGZipfile(text2, text);
                _ = InitCvtDesGUI(text);
                if (File.Exists(text))
                {
                    File.Delete(text);
                }
    ...
    public void CheckAPIVerforEVB_Bin()
    {
        if (m_DbgParser.IsConnected())
        {
            ...
            GuiItem guiItem = GetGuiGroupByItemName("Major").FindItemByName("Major");
    ...
    public void CheckAPIVerforEVB_XML()
    {
        if (m_DbgParser.IsConnected())
        {
            ...
            _ = int.Parse(m_ispVersion.Split('.').ElementAt(1));
    ...
    public int LoadBinFile(string filePath)
    {
        ...
        if (m_DbgParser.IsConnected())
        {
            ...
            _ = 1835336475;
            if (chipIDName is >= ChipID.I3 or ChipID.M5 or ChipID.M5U)
            {
                ...
                if (chipIDName is not ChipID.M5 and not ChipID.M5U and not ChipID.I6E and not ChipID.I6B0 and not ChipID.P3)
                {
                ...
                if (BinChipID != chipID)
                {
                    string text = "[LoadBinFile] ChipID is not match. Bin  ChipID: " + Binchipname + ".  EVB chipID: " + chipname + ".\n";
                    ShowMessage(text, "");
                    APIVerIsMatch = false;
                    return 1;
                }
                ...
                if (BinAPIVerMajor != EvbAPIVerMajor)
                {
                    string text2 = "Major API Version  is not match. Bin  Major: " + BinAPIVerMajor + ".  EVB Major: " + EvbAPIVerMajor + ".\n";
                    ShowMessage(text2, "");
                    APIVerIsMatch = false;
                    result = 1;
                }
        ...
        if (num2 == 0)
        {
            ...
            if ((int)binaryReader.BaseStream.Length > num + num3)
            {
                try
                {
                    ...
                    BinaryWriter binaryWriter = new(File.Open(ZipPath, FileMode.Create));
                    binaryWriter.Write(buffer2);
                    binaryWriter.Close();
            ...
            if ((int)binaryReader.BaseStream.Length > num + num4)
            {
                try
                {
                    ...
                    if (CRC32.CheckCRC32(array, checkNum, m_BinChecksumVer))
                    {
                        SrcPath = Application.StartupPath + "/XmlTmp.xml";
                        ZipPath = Application.StartupPath + "/XmlTmp.zip";
                        BinaryWriter binaryWriter2 = new(File.Open(ZipPath, FileMode.Create));
    ...
    private void GetApiVersionForI3()
    {
        ...
        if (m_DbgParser.ReceiveApiPacket(apiIDValue, out byte[] pbyRcvData) > 0)
        {
    ...
    private void GetApiVersionForM5()
    {
        ...
        if (m_DbgParser.ReceiveUSBInitialApiPacket(apiIDValue, bufferInitial, out byte[] pbyRcvData) > 0)
        {
            PacketParser.ConvertBufferToTypeForI3(ref EvbAPIVerMajor, ref EvbAPIVerMinor, pbyRcvData);
        }
    }
    ...
    public int LoadBinXmlFile(string filePath)
    {
        ...
        if (m_DbgParser.IsConnected())
        {
            ...
            if (chipIDName is >= ChipID.I3 or ChipID.M5 or ChipID.M5U)
            {
                ChipID chipID = chipIDName switch
                {
                    ChipID.I6E or ChipID.I6B0 or ChipID.P3 => ChipID.I6E,
                    ChipID.M5 or ChipID.M5U => ChipID.M5,
                    _ => ChipID.I6,
                };
                if (chipIDName is not ChipID.M5 and not ChipID.M5U)
                {
                    GetChipIDByAPI(ref chipID);
                }
                GetBinFileChip(filePath);
                ...
                if (BinChipID != chipID)
                {
                    string text = "[LoadBinXmlFile] ChipID is not match. Bin  ChipID: " + Binchipname + ".  EVB chipID: " + chipname + ".\n";
                    ShowMessage(text, "");
                    if (File.Exists(SrcPath))
                    {
                        File.Delete(SrcPath);
    ...
    public int checkChipID(ChipID chipID, string filePath)
    {
        ...
        if (XmlChipID != chipID)
        {
            ...
            else
            {
                ...
                string text = "[CheckChipID] ChipID is not match. Xml  ChipID: " + Xmlchipname + ".  EVB chipID: " + chipname + ".\n";
                ShowMessage(text, "");
                APIVerIsMatch = false;
                result = 1;
            }
    ...
    public int checkAPIVersion(string filePath, ChipID chipID, CONNECT_MODE mode)
    {
        ...
        {
            string text = "Major API Version  is not match. Xml  Major: " + XmlAPIVerMajor + ".  EVB Major: " + EvbAPIVerMajor + ".\n";
            ShowMessage(text, "");
            APIVerIsMatch = false;
            result = 1;
        }
    ...
    public int LoadXmlFile(string filePath)
    {
        ...
        if (m_DbgParser.IsConnected())
        {
            ...
            if (chipIDName is >= ChipID.I3 or ChipID.M5 or ChipID.M5U)
            {
                ...
                if (XmlChipID != chipID)
                {
                    string text = "[LoadXmlFile] ChipID is not match. Xml  ChipID: " + Xmlchipname + ".  EVB chipID: " + chipname + ".\n";
                    ShowMessage(text, "");
                    APIVerIsMatch = false;
                    return 1;
                }
                ...
                if (XmlAPIVerMajor != EvbAPIVerMajor)
                {
                    string text2 = "Major API Version  is not match. Xml  Major: " + XmlAPIVerMajor + ".  EVB Major: " + EvbAPIVerMajor + ".\n";
                    ShowMessage(text2, "");
                    APIVerIsMatch = false;
                    result = 1;
                }
