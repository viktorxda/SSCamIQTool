...
public class FormShading : Form
{
    ...
    private readonly long[] arData;

    private readonly GuiGroup m_GridGuiGroup;

    private readonly GuiItem m_GridGuiItem;

    private readonly IQComm m_DbgComm;

    private readonly int nColumn;

    private readonly int nRow;

    private readonly int nWidth = 61;

    private readonly int nHeight = 69;
    ...
    private readonly DataSet set = new();
    ...
    public FormShading(GuiGroup group, IQComm comm, string strTag)
    {
        ...
        ParseGridHeader(m_GridGuiItem.Paramters, nColumn, nRow, out _, out _);
        ...
        {
            _ = comboBoxSelect.Items.Add("table" + i);
    ...
    private void panelForColor_Paint(object sender, PaintEventArgs e)
    {
        ...
        for (int i = 0; i < 360; i++)
        {
            Rectangle rect = new(670 + i, 10, 1, 20);
            HsvToRgb(i, 1.0, 1.0, out int r, out int g, out int b);
            SolidBrush brush = new(Color.FromArgb(r, g, b));
    ...
    protected void UpdateTableValue()
    {
        ...
        _ = nColumn;
        _ = labelMultiCtrlTitle.Text.Split('>');
        for (int i = 0; i < nRow; i++)
        {
    ...
    public void ReadTableTask(object obj)
    {
        ...
        if (m_GridGuiGroup.ReadGroup(m_DbgComm).Equals(""))
        {
            ...
            _ = Invoke(new UpdateTableHandler(ShowColor));
            _ = Invoke(new ShowMessageHandler(ShowMessageBox), "Read shading success");
        }
        else
        {
            _ = Invoke(new ShowMessageHandler(ShowMessageBox), "Read shading failed");
        }
    }
    ...
    private void SendTableTask(object obj)
    {
        ...
        {
            _ = Invoke(new ShowMessageHandler(ShowMessageBox), "Write shading failed");
        }
    }
    ...
    protected void ShowColor()
    {
        for (int i = 0; i < nHeight; i++)
        {
            for (int j = 1; j < nWidth + 1; j++)
            {
                ...
                HsvToRgb(num, 1.0, 1.0, out int r, out int g, out int b);
